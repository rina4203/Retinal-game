...
@version 1.8 (Optimized Rendering)
@date 2025
"""
...
# --- FILE PATHS ---
SOUND_FOLDER = "song"             #< ВИПРАВЛЕНО: Тепер папка називається song
FONT_FOLDER = "etc"              #< Directory containing font files
...
def draw_text(surf, text, font, x, y, color, align="center"):
    ...
    cache_key = (text_str, id(font), color_tuple)
    
    if cache_key not in _text_cache:
        _text_cache[cache_key] = font.render(text_str, True, color_tuple)
        
    text_surface = _text_cache[cache_key]
...
def get_cached_star_surface(size_int, base_color, halo_color, halo_alpha):
    """Caches star halo surfaces to avoid recreating SRCALPHA surfaces every frame."""
    s = pygame.Surface((size_int * 4, size_int * 4), pygame.SRCALPHA)
    pygame.draw.circle(s, (*halo_color, halo_alpha), (size_int * 2, size_int * 2), int(size_int * 1.6))
    pygame.draw.circle(s, (*base_color, 255), (size_int * 2, size_int * 2), size_int)
    return s

@functools.lru_cache(maxsize=256)
def get_cached_trail_surface(t_size, base_color, alpha):
    """Caches small trail circles."""
    s_trail = pygame.Surface((t_size * 2, t_size * 2), pygame.SRCALPHA)
    pygame.draw.circle(s_trail, (*base_color, alpha), (t_size, t_size), t_size)
    return s_trail

@functools.lru_cache(maxsize=128)
def get_cached_cloud_surface(radius, alpha):
    """Caches procedural clouds."""
    surf_size = radius * 3
    cloud_surf = pygame.Surface((surf_size, surf_size), pygame.SRCALPHA)
    center = (surf_size // 2, surf_size // 2)
    pygame.draw.circle(cloud_surf, (255, 255, 255, int(alpha * 0.4)), center, int(radius * 1.5))
    pygame.draw.circle(cloud_surf, (255, 255, 255, alpha), center, int(radius * 0.7))
    return cloud_surf
...
class ProceduralWave(GameObject):
    ...
    def draw(self, surface):
        if self._type == "wave":
            ...
            points = []
            # Optimization: Step by 5 pixels instead of 1 (reduces math.sin calls significantly)
            for x in range(0, WIDTH + 6, 5):
            ...
            pulse = int(math.sin(self._phase * self._wobble_speed) * 3)
            current_radius = int(self._radius) + pulse
            cloud_surf = get_cached_cloud_surface(current_radius, self._alpha)
            surf_size = current_radius * 3
...
class Particle(GameObject):
    ...
    def draw(self, surface):
        if self._size > 0: pygame.draw.circle(surface, self._color, (int(self._x), int(self._y)), int(self._size))
...
class Star(GameObject):
    ...
    def get_color(self):
        return tuple(self._specific_color) if self._specific_color else PASTEL_CREAM
    ...
    def draw(self, surface):
        ...
        # Округлюємо розмір для зменшення варіацій в кеші
        current_size = int(self._base_size * (0.8 + p * 0.4)) 
        ...
        base_col_tuple = tuple(base_color)
        halo_col_tuple = tuple(halo_color)

        if self._is_rhythm_note:
             if len(self._trail) > 1:
                trail_len = len(self._trail)
                for i, (tx, ty) in enumerate(self._trail):
                    alpha = int(150 * (i / trail_len))
                    t_size = max(1, int(self._base_size * (i / trail_len) * 0.8))
                    s_trail = get_cached_trail_surface(t_size, base_col_tuple, alpha)
                    surface.blit(s_trail, (tx - t_size, ty - t_size))
                    
             s = get_cached_star_surface(current_size, base_col_tuple, halo_col_tuple, halo_alpha_base)
            ...
            if shape == "Square":
                ...
                s = get_cached_star_surface(current_size, base_col_tuple, halo_col_tuple, outer_alpha)
                surface.blit(s, (self._x - current_size*2, self._y - current_size*2))
    ...
    def handle_event(self, event):
        ...
        if self._current_view == "main":
            if self._btn_to_audio.check_click(event): self._current_view = "audio"
                ...
                loc.toggle_lang()
                self._game._load_fonts() 
                self._btn_lang.set_text_key("LANG")
                _text_cache.clear() # Очистка кешу тексту при зміні мови
    ...
    def update(self, dt):
        ...
        for star in self._star_manager.get_list()[:]:
            if star.get_y() > HEIGHT:
                ...
                if note_name:
                    self._game.play_note_sound(note_name) 
...
class TestRetinalSystems(unittest.TestCase):
    ...
    def test_highscore_update(self):
        ...
        self.assertEqual(self.game.get_high_score(), 100) 
        self.game.update_high_score(200)
        self.assertEqual(self.game.get_high_score(), 200) 
